...
function initLoginPage() {
  ...
  sendBtn.addEventListener('click', async () => {
    if (sendInProgress) return;  // prevent double-click / double-fire
    const id = input.value.trim();
    if (!id) return showToast('Enter your email address', 'error');
    if (!id.includes('@')) return showToast('Please enter a valid email address', 'error');
    sendInProgress = true;
    ...
    if (data.success) {
      if (data.is_returning) {
        ...
        if (subtitle) {
          subtitle.textContent = '👋 Welcome back! Enter your verification code';
      ...
      hint.innerHTML = `📧 Code sent to <strong>${id}</strong>. Check your inbox (and spam folder).`;
