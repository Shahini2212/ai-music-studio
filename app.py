import os, time, io, requests as http_requests
...
def hf_generate_music(prompt, duration=20):
    ...
    # Gradio 6 uses /run/generate endpoint (based on api_name="generate")
    endpoints_to_try = [
        f"{MY_SPACE_URL}/run/generate",
        f"{MY_SPACE_URL}/run/predict",
        f"{MY_SPACE_URL}/api/predict",
    ]

    for endpoint in endpoints_to_try:
        try:
            print(f"[HF API] Trying: {endpoint}")
            payload = {"data": [prompt, duration]}
            ...
            if resp.status_code == 200:
                ...
                if data:
                    audio_info = data[0]
                    # Gradio returns audio as dict with 'url'
                    ...
                    # Gradio numpy format [sample_rate, data_array]
            ...
            print(f"[HF API] {endpoint} → {resp.status_code}: {resp.text[:150]}")

        except Exception as e:
            print(f"[HF API] {endpoint} exception: {e}")
...
def video_bgm():
    if not HF_TOKEN:
        return jsonify({'success': False, 'message': 'HF_TOKEN not configured'}), 500
    ...
    try:
        audio_bytes = hf_generate_music(final_prompt, duration)
        if not audio_bytes:
            return jsonify({'success': False, 'message': 'Music generation failed. Check HF_TOKEN.'}), 500

        out_filename = f"bgm_{int(time.time())}.wav"
        save_path    = os.path.join('static', 'generated', out_filename)
        with open(save_path, 'wb') as f:
            f.write(audio_bytes)

        audio_url = url_for('static', filename=f"generated/{out_filename}")
        title     = f"BGM: {mood[:25]}..."
...
def send_otp_email(email, otp):
    ...
    <div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;padding:32px;
      ...
      <div style="background:#1a1a2e;border:2px solid #7c3aed;border-radius:12px;
        ...
        <h1 style="font-size:2.8rem;letter-spacing:18px;color:#00d9ff;margin:0;
                   font-family:monospace;">{otp}</h1>
      </div>
      <p style="color:#aaa;font-size:0.9rem;">Enter this code on the login page to continue.</p>
...
def send_otp_sms(phone, otp):
    """Send OTP via Twilio SMS. Set TWILIO_SID, TWILIO_TOKEN, TWILIO_FROM in env vars."""
    ...
    try:
        ...
        client.messages.create(
            body=f"Your AI Music Studio verification code is: {otp}. Valid for 10 minutes.",
            from_=from_,
            to=phone
        )
        return True
...
def is_phone(identifier):
    ...
    import re
    return bool(re.match(r'^\+?[\d\s\-]{7,15}$', identifier))
...
def send_code():
    ...
    if is_phone(identifier):
        sent = send_otp_sms(identifier, otp)
        medium = 'phone'
    else:
    ...
    if sent:
        print(f"[OTP SENT via {medium}] to {identifier}")
        return jsonify({'success': True, 'is_returning': existing is not None, 'medium': medium})
    else:
        # Fallback: still works but OTP only in server logs (never shown on screen)
        print(f"[OTP FALLBACK - check server logs] {identifier}: {otp}")
        return jsonify({'success': True, 'is_returning': existing is not None, 'medium': 'log',
                        'warning': 'Email/SMS not configured — check server logs'})
...
def generate_music():
    ...
    try:
        ...
        audio_bytes = hf_generate_music(final_prompt, duration)
        if not audio_bytes:
            return jsonify({'success': False, 'message': 'Music generation failed. Check HF_TOKEN is set correctly.'}), 500
        gen_time = round(time.time() - gen_start, 1)

        out_filename = f"gen_{int(time.time())}.wav"
        save_path    = os.path.join('static', 'generated', out_filename)
        with open(save_path, 'wb') as f:
            f.write(audio_bytes)
...
""")
        ...
        return jsonify({'success': True, 'track': {
            ...
            'device': dev,
            'duration': duration,
            'model': 'MusicGen-Small via HF API',
...
""")
        ...
        # Step 3: Generate music via HF API
        gen_start = time.time()
        audio_bytes = hf_generate_music(music_prompt, duration)
        if not audio_bytes:
            return jsonify({'success': False, 'message': 'Music generation failed. Check HF_TOKEN.'}), 500
        gen_time = round(time.time() - gen_start, 1)

        out_filename = f"lyrics_{int(time.time())}.wav"
        save_path = os.path.join('static', 'generated', out_filename)
...
def remix_songs():
    ...
    try:
        ...
        for i, log in enumerate(transition_log):
            ...
            try:
                bridge_bytes = hf_generate_music(dj_prompt, bridge_dur)
                if not bridge_bytes:
                    raise Exception("HF API returned no audio")
                bridge_wav_path = os.path.join('static', 'generated', f"bridge_{int(time.time())}.wav")
                with open(bridge_wav_path, 'wb') as bf:
                    bf.write(bridge_bytes)
                from pydub import AudioSegment as _AS
                bridge_seg = _AS.from_wav(bridge_wav_path)
...
def video_bgm_replace():
    ...
    try:
        ...
        bgm_filename = f"bgm_{int(time.time())}.wav"
        bgm_wav_path = os.path.join('static', 'generated', bgm_filename)
        with open(bgm_wav_path, 'wb') as bf:
